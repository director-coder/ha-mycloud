...
def core_get_state(entity_id: str):
    ...
    test = requests.get(f"{CORE_BASE}/api/config", headers=HEADERS, timeout=10)
    print(
...
def supervisor_get_mounts():
    # debug (можно потом убрать)
    r = requests.get(f"{SUPERVISOR_BASE}/info", headers=HEADERS, timeout=10)
...
async function api(path, opts){
  ...
  if(!r.ok){
    // если backend вернул {error: "..."} — покажем именно его
    ...
    actions.appendChild(
      btn('Mount', async()=>{ 
        try {
          ...
          if (msg.includes('authorization') || msg.includes('access denied') || msg.includes('logon') || msg.includes('permission denied')) {
...
def api_shares():
    ...
    for m in mounts:
        existing_mount_names.add(m.get("name", ""))
...
def api_mount():
    ...
    mounts_data = supervisor_get_mounts()
    existing_names = {m.get("name", "") for m in mounts_data.get("mounts", [])}

    mount_name = build_mount_name("wdmc", share_name, existing_names)

    protocol = opt.get("protocol", "cifs")
    if protocol != "cifs":
        return jsonify({"error": "Only CIFS implemented in this add-on right now"}), 400
    ...
    if not r.ok:
        ...
        return jsonify({"error": (txt_raw[:500] if txt_raw else f"Mount failed with status {r.status_code}")}), 500
